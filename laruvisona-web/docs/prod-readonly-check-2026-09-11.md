...
## 記入前の状態（2026-09-11 時点）

**C-1〜C-5 はすべて未確認。** この作業環境は本番に接続しておらず、
GitHub にも届かない（社内プロキシが 403 を返す）。下の値は**ローカルで読めた事実だけ**で、
本番で何が動いているかの答えではない。

| | 状態 | 理由 |
|---|---|---|
| C-1 (a) GitHub main の先端 | **未確認** | この環境から GitHub へ到達できない |
| C-1 (b) Render が動かしているSHA | **未確認** | Render の管理画面を見ていない |
| C-2 保存済みHTMLの版の分布 | **未確認** | 本番Supabaseに接続していない |
| C-3 `site_domains` の適用状態 | **未確認** | 同上 |
| C-4 設定の有無 | **未確認** | Render の Environment を見ていない |
| C-5 `REPUBLISH_ON_BOOT` が無いこと | **未確認** | 同上 |
...
| | 記入 |
|---|---|
| `99` が false のときの扱い | 是正する / 例外として進める |
| 例外にした場合の理由 | ______________________________ |
| 判断した人・日付 | ______________________________ |

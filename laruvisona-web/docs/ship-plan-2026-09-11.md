...
| `99 望ましい状態` | 98 に「移行SQLの積み残し」（27・28行目）を足したもの。**公開の可否はこちらで見る** |

**`98=true` は「独自ドメインを公開してよい」という意味ではない。**
`99=false` のあいだは、次のどちらかが済むまで**独自ドメインの有効化を始めない**。

1. 残っている権限を取り消す移行SQLを当てて `99=true` にする
2. 残したまま進める理由と、判断した人・日付を記録シートに書いて、**例外として明示する**
...
一時PostgreSQLを立て、**正常・未適用・22通りの異常**を作って、期待どおりの行が落ちるかを見る。
...
| 状態 | 98 | 99 |
|---|---|---|
| 移行SQLを当てた直後 | true | false（27・28が残る） |
| 隔離環境で残存権限まで是正 | true | true |
| 解除キューに `TRUNCATE` を付与 | false | false |
| 未適用 | false | false（98行・99行が1行ずつ出ることも確認） |

ロールは**データベース単位ではなくクラスタ共有**なので、回帰スクリプトは
ロールの作成をクラスタで1回だけ行い、ロールを変えるケースは必ず元へ戻し、
**ケースごとに開始時の基準状態（98=t / 99=f / 落ちる行は 27・28 だけ）を確認**してから
...
- 7b で `98=false`（移行が未了）
- 7b で `99=false` のまま独自ドメインを有効化しようとしている
  ―― 権限を是正して `99=true` にするか、残す理由と判断した人・日付を
  記録シートに書いて例外として明示するまで、独自ドメインは始めない

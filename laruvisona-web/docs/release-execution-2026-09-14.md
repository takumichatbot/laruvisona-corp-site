...
- LARUbotフォームへ「LARU HP 本番確認」と明記した問い合わせを1件送信し、完了画面まで到達した。会社名・氏名・メール・本文の必須経路を実際に通した。受信箱での着信確認は別途行う
- 専用チャットボットの作成はLARUbot管理画面への認証が必要。認証済みセッションが無かったため、専用IDを設定するまではLARU HP上のチャットランチャーを出さず、会社名のボットを誤表示しない

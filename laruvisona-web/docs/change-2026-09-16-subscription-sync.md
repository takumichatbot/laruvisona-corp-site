...
```json
{"dryRun":true,"scanned":0,"complete":true,"inSync":0,
 "fixed":[],"stopped":["fdc23d45-…"],"conflicts":[],"unlinked":[]}
```

- 本番Stripeに契約は**0件**（`STRIPE_SECRET_KEY` は本番鍵である）
- DBに1件だけ `active` の人が居た。その `stripe_subscription_id` を
  Stripeで引くと「**テスト環境でのみご利用可能です**」と出る。
  つまりテストモードの契約が本番DBへ書かれたまま残っていた

### 一斉停止に歯止めを足した

この結果を見て、逆向き（DBだけ有効→canceled）の危うさに気づいた。
**Stripeの契約が1件も無いのにDBには有効な人が居る**という状況は、
「全員が解約した」よりも「鍵の環境（test/live）が入れ替わっている」
ほうがずっとありそうである。その状態で走らせると、実際に払っている人を
全員止めてしまう。

そこで、Stripeの契約が0件のときは停止を行わず、理由を返すようにした。
...
分かったうえで進めるときだけ、本文に
`{"stopWithoutStripeSubscriptions": true}` を付ける。

...
with checks(section,item,ok) as (
  ...
  ('booking-payment','hp_payment_accounts',to_regclass('public.hp_payment_accounts') is not null),
  ('booking-payment','hp_booking_payments',to_regclass('public.hp_booking_payments') is not null),
  ('booking-payment','change RPC',to_regprocedure('public.hp_schedule_change(uuid,uuid,text,uuid,text,timestamp with time zone,text,integer)') is not null),
  ('booking-payment','payment prepare RPC',to_regprocedure('public.hp_payment_prepare(uuid,uuid,text,text)') is not null),
  ('booking-payment','payment attach RPC',to_regprocedure('public.hp_payment_attach(uuid,uuid,text)') is not null),
  ('booking-payment','payment settle RPC',to_regprocedure('public.hp_payment_settle(uuid,uuid,text,text,text,integer)') is not null),

...
psql -q -d "$DB" -v ON_ERROR_STOP=1 -f "$DIR/hp_reservations.sql"
psql -q -d "$DB" -v ON_ERROR_STOP=1 -f "$DIR/hp_reservations_reminded.sql"
...
psql -q -d "$DB" -v ON_ERROR_STOP=1 -f "$DIR/hp_public_rate_limits.sql"
...
psql -d "$DB" -v ON_ERROR_STOP=1 -f "$DIR/hp_scheduling_regression.sql"
psql -d "$DB" -v ON_ERROR_STOP=1 -f "$DIR/hp_public_rate_limits_regression.sql"
psql -d "$DB" -v ON_ERROR_STOP=1 -f "$DIR/hp_analytics_regression.sql"

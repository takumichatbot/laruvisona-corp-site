...
export async function POST(req: Request) {
  ...
  const rate = await claimPublicRate(service, 'newsletter-subscribe', `${input.siteId}:${clientIp(req)}`, 5);
  if (rate === 'limited') return NextResponse.json({ error: 'リクエストが多すぎます。しばらくしてから再試行してください' }, { status: 429 });
  if (rate === 'unavailable') return NextResponse.json({ error: '登録受付を確認できません。時間をおいてください' }, { status: 503 });

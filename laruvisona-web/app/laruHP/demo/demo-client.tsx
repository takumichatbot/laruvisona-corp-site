    ...
    <div className="min-h-screen bg-sky-50">
      ...
      <header className="fixed top-0 w-full z-50 bg-white/90 backdrop-blur-xl border-b border-sky-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <Link href={`${LARUHP_APP_ORIGIN}/laruHP/auth/signup`} className="bg-sky-600 text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-sky-500 transition-all">
            {PRIMARY_CTA.label} →
          </Link>
        </div>
      </header>
                ...
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
                  ...
                  <div className={`bg-gradient-to-br ${selected.color} p-6`}>
                    ...
                    <div className="mt-4 flex items-center gap-2 bg-indigo-600 text-white rounded-xl px-3 py-2 text-xs">
                      ...
                      <span>チャットで来訪者の質問に応対（Lite以上のプラン）</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <p className="text-gray-500 text-sm leading-relaxed">
                      業種と屋号を入れると、この構成で下書きが作られます。
                      文章・写真・配色は、完成した見た目のまま直せます。
                    </p>
                  </div>
                  ...
                  <Link
                    href={`${LARUHP_APP_ORIGIN}/laruHP/studio?industry=${selected.id}`}
                  ...
                  <p className="text-center text-xs text-gray-400">
                    登録は無料です。作りはじめるにはプランの申し込みが必要で、月払いは初月0円（{TERMS.minimumMonths}ヶ月の最低利用期間があります）。
                  </p>
                </div>
              </div>
            </>

...
// 道しるべ（23本のリンク）は lib/laruhp-nav.ts へ移した。
// 本文の中ほど（サイト一覧の見出しと中身のあいだ）に挟まっていて、
// しかもこの画面にしか無かった。骨組み側に置けば、どの画面からも同じ場所にある。
...
export default function DashboardPage() {
  ...
  return (
    <AppShell
      title="ホーム"
      lead={sites.length ? `${sites.length}件のサイト・公開中 ${sites.filter(x => x.published).length}件` : undefined}
      unreadContacts={unread}
      email={userEmail}
      actions={headerActions}
    >
    <div className="laru-touch">
